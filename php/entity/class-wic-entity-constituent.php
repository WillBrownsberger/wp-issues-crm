...
class WIC_Entity_Constituent extends WIC_Entity_Parent {
	...
	protected function form_update ( $args ) {
		$this->fields = WIC_DB_Dictionary::get_form_fields( $this->entity );
		$this->initialize_data_object_array_from_submitted_form();
		$update = WIC_DB_Access_Factory::make_a_db_access_object( $this->entity );
		$update->update ( $this->data_object_array ); 
		if ( false === $update->outcome ) {
			$message = __( 'Please correct form errors: ', 'wp-issues-crm' ) . $update->explanation;
			$message_level = 'error';
			$update_form = new WIC_Form_Constituent_Update;
			$update_form->layout_form ( $this->data_object_array, $message, $message_level );
			if ( $update->found_count > 1 || ( ( 1 == $this->found_count ) && ( $update->result[0]->ID != $this->data_object_array[ID]->get_value() ) ) ) {	
				$lister = new WIC_List_Parent;
				$list = $lister->format_entity_list( $update, false );
				echo $list;
			}	
		} else {
			$message = __( 'Update successful.  You can update again. ', 'wp-issues-crm' ) . $update->explanation;
			$message_level = 'guidance';
			$update_form = new WIC_Form_Constituent_Update;
			$update_form->layout_form ( $this->data_object_array, $message, $message_level );					
		}
